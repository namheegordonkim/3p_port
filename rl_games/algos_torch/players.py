...
class PpoPlayerDiscrete(BasePlayer):
    def __init__(self, config):
        ...
        obs_shape = self.obs_shape
        config = {"actions_num": self.actions_num, "input_shape": obs_shape, "num_seqs": self.num_agents, "value_size": self.value_size}
    ...
    def get_masked_action(self, obs, action_masks, is_determenistic=True):
        ...
        input_dict = {"is_train": False, "prev_actions": None, "obs": obs, "action_masks": action_masks, "rnn_states": self.states}
        self.model.eval()
...
class SACPlayer(BasePlayer):
    def __init__(self, config):
        ...
        self.network = config["network"]
        self.actions_num = self.action_space.shape[0]
        self.action_range = [float(self.env_info["action_space"].low.min()), float(self.env_info["action_space"].high.max())]

        obs_shape = torch_ext.shape_whc_to_cwh(self.state_shape)
        self.normalize_input = False
        config = {"obs_dim": self.env_info["observation_space"].shape[0], "action_dim": self.env_info["action_space"].shape[0], "actions_num": self.actions_num, "input_shape": obs_shape}
        self.model = self.network.build(config)
        self.model.to(self.device)
        self.model.eval()
        self.is_rnn = self.model.is_rnn()
