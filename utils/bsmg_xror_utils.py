...
def open_xror(filename):
    # XROR processing
    beatmap, song_info = open_bsmg_or_boxrr(None, filename)
...
def open_bsmg(filename, logger):
    # BSMG processing
    beatmap, song_info = open_bsmg_or_boxrr(filename, None, logger)
    notes_np, bombs_np, obstacles_np = get_xbo_np(beatmap, song_info)
    song_duration = notes_np[:, 0].max()  # in seconds
    timestamps = np.arange(0, song_duration + 5, 1 / 60)
    d = {
        "notes_np": notes_np,
        "bombs_np": bombs_np,
        "obstacles_np": obstacles_np,
