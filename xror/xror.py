...
class XROR:
    ...
    def __init__(self, id=None, name=None, timestamp=None):
        ...
        """
        self.data = {"$schema": "https://metaguard.github.io/xror/schema/v1.0.0/schema.json"}
        if id:
            self.data["$id"] = id
        self.data["info"] = {}
        if name:
        ...
        if timestamp:
            self.data["info"]["timestamp"] = int(timestamp)
    ...
    def setApp(self, id=None, name=None, version=None):
        ...
        """
        if "software" not in self.data["info"]:
            self.data["info"]["software"] = {}
        app = {}
        if id:
            app["id"] = id
        if name:
    ...
    def addExtension(self, id=None, name=None, version=None):
        ...
        if "software" not in self.data["info"]:
            self.data["info"]["software"] = {}
        if "app" not in self.data["info"]["software"]:
            self.data["info"]["software"]["app"] = {}
        if "extensions" not in self.data["info"]["software"]["app"]:
            self.data["info"]["software"]["app"]["extensions"] = []
    ...
    def addEventType(self, id=None, name=None, attr=None, floatData=None, otherData=None):
        ...
        if attr:
            ev["attr"] = attr
        if floatData:
            ev["floatData"] = floatData
        if otherData:
            ev["otherData"] = otherData
    ...
    def addEvent(self, type, time, floatData=[], otherData=None):
        ...
        for i in range(len(self.data["events"])):
            if self.data["events"][i]["id"] == type:
                idx = i
        if "floatData" not in self.data["events"][idx]:
            self.data["events"][idx]["floatData"] = []
        self.data["events"][idx]["floatData"].append([time] + floatData)
        if otherData:
            if "otherData" not in self.data["events"][idx]:
                self.data["events"][idx]["otherData"] = []
    ...
    def getEvents(self, type):
        ...
        for i in range(len(self.data["events"][idx]["floatData"])):
            if self.data["events"][idx]["floatData"][i][0] == -1:
                continue
            event = {}
    ...
    def pack(self):
        ...
        if "timestamp" in data["info"]:
            data["info"]["timestamp"] = datetime.datetime.fromtimestamp(data["info"]["timestamp"], datetime.timezone.utc)
    ...
    def toBSOR(self):
        ...
        if "modifiers" in self.data["info"]["software"]["activity"]:
            addString(self.data["info"]["software"]["activity"]["modifiers"])
        else:
            addString("")
        if "jumpDistance" in self.data["info"]["software"]["activity"]:
            addFloat(self.data["info"]["software"]["activity"]["jumpDistance"])
        else:
            addFloat(0.0)
        if "leftHanded" in self.data["info"]["software"]["activity"]:
            addByte(self.data["info"]["software"]["activity"]["leftHanded"])
        else:
            addByte(False)

        for attr in ["height", "startTime", "failTime", "speed"]:
            if attr in self.data["info"]["software"]["activity"]:
                addFloat(self.data["info"]["software"]["activity"][attr])
            else:
                addFloat(0)
        ...
        notes = sum([gc, bc, m, b], [])
        if "order" in notes[0]:
            notes = sorted(notes, key=lambda d: d["order"])
        else:
        ...
        for note in notes:
            ...
            if note["eventType"] == 0 or (note["eventType"] == 1 and self.data["info"]["software"]["runtime"] != "oculus"):
                for attr in ["speedOK", "directionOK", "saberTypeOK", "wasCutTooSoon"]:
                    addByte(note[attr])
                for attr in ["saberSpeed", "saberDirX", "saberDirY", "saberDirZ"]:
                    addFloat(note[attr])
    ...
    def fromTilt(XROR, data):
        ...
        if "EnvironmentPreset" in meta:
            xror.setEnvironment(id=meta["EnvironmentPreset"])
        for attr in [
        ...
        xror.addEventType(
            id="stroke",
            name="Stroke",
            attr=["color_r", "color_g", "color_b", "color_a", "size", "scale", "brush"],
        )
    ...
    def fromBSOR(XROR, data, addFPS=False, addOrder=False, addGarbage=False):
        ...
        xror.addDevice(
            name=bsor.info.controller.replace("right", "left").replace("Right", "Left").replace("RIGHT", "LEFT"),
            type="CONTROLLER",
            joint="HAND_LEFT",
        )
        ...
        for attr in [
            ...
            if getattr(bsor.info, attr):
                xror.data["info"]["software"]["activity"][attr] = getattr(bsor.info, attr)
        ...
        if addOrder:
            ...
            if bsor.info.platform == "oculus":
                ...
                if addGarbage:
                    xror.addEventType(id="g", name="Garbage", attr=["data"])
            else:
                xror.addEventType(
                    id="bc",
                    name="Bad Cut",
            ...
            if note.event_type == 0:
                xror.addEvent(
                    ...
                    [
                        note.spawn_time,
                        note.cut.saberSpeed,
                        note.cut.saberDirection[0],
                        note.cut.saberDirection[1],
                        note.cut.saberDirection[2],
                        note.cut.timeDeviation,
                        note.cut.cutDeviation,
                        note.cut.cutPoint[0],
                        note.cut.cutPoint[1],
                        note.cut.cutPoint[2],
                        note.cut.cutNormal[0],
                        note.cut.cutNormal[1],
                        note.cut.cutNormal[2],
                        note.cut.cutDistanceToCenter,
                        note.cut.cutAngle,
                    ],
                    (
                        [
                            note.cut.beforeCutRating,
                            note.cut.afterCutRating,
                            note.note_id,
                            note.cut.saberType,
                            idx,
                        ]
            ...
            if note.event_type == 1:
                if bsor.info.platform == "oculus":
                    xror.addEvent(
                        ...
                        note.event_time,
                        [note.spawn_time],
                        [note.note_id, idx] if addOrder else [note.note_id],
                    )
                    if addGarbage:
                        xror.addEvent("g", note.event_time, [], [note.garbage])
                else:
                    xror.addEvent(
                        "bc",
                        note.event_time,
                        ...
                        (
                            [
                                ...
                                note.note_id,
                                note.cut.speedOK,
                                note.cut.directionOk,
                                note.cut.saberTypeOk,
                                note.cut.wasCutTooSoon,
                                note.cut.saberType,
                                idx,
                            ]
                            if addOrder
