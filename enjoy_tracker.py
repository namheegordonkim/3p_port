...
class AppState:
    def __init__(
        ...
        self.rb_pos = rb_pos
        self.rb_rot = rb_rot
        self.skel_visuals = skel_visuals
...
def setup_and_run_gui(pl: ImguiPlotter, app_state: AppState):
    ...
    def gui():
        ...
        for i in range(app_state.skel_visuals.shape[0]):
            skel_visual = app_state.skel_visuals[i]
            m = np.eye(4)
            pos = app_state.rb_pos[0, app_state.frame, i]
            quat = app_state.rb_rot[0, app_state.frame, i]
            m[:3, 3] = pos
            m[:3, :3] = Rotation.from_quat(quat).as_matrix()
            skel_visual.actor.user_matrix = m
            skel_visual.actor.SetVisibility(True)

            # Sabers
            if i in [18, 23]:
                saber_idx = {18: 0, 23: 1}[i]
                ...
                app_state.saber_visuals[saber_idx].actor.user_matrix = m
                app_state.saber_visuals[saber_idx].actor.SetVisibility(True)
                if app_state.collide_yes_across_time[0, app_state.frame, saber_idx].any():
                    saber_color = np.array([[1.0, 0.0, 0.0]])
                    if app_state.good_yes_across_time[0, app_state.frame, saber_idx].any():
                        saber_color = np.array([[0.0, 1.0, 0.0]])
                else:
                    saber_color = colors[saber_idx]
                app_state.saber_visuals[saber_idx].mesh.cell_data["color"] = saber_color.repeat(app_state.saber_visuals[saber_idx].mesh.n_cells, 0) * 1
...
def main(args, remaining_args):
    ...
    in_boxrr = args.in_boxrr
    in_posrot = args.in_posrot
    d = torch.load(in_posrot)
    rb_pos = d["rb_pos"]
    rb_rot = d["rb_rot"]

    skel_visual = XMLVisualDataContainer(f"data/assets/mjcf/smpl_0_humanoid.xml")
    skel_visuals = np.empty(len(skel_visual.meshes), dtype=object)
    for i, mesh in enumerate(skel_visual.meshes):
        ...
        skel_visuals[i] = GenericVisual(mesh, actor)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--in_posrot", type=str, required=True)
