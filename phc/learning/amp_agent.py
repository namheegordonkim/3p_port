...
class AMPAgent(common_agent.CommonAgent):
    ...
    def __init__(self, base_name, config):
        ...
        self.temp_running_mean = (
            self.vec_env.env.task.temp_running_mean
    ...
    def train_epoch(self):
        ...
        for _ in range(0, self.mini_epochs_num):
            ...
            for i in range(len(self.dataset)):
                ...
                # curr_train_info = self.train_actor_critic(self.dataset[0])
    ...
    def pre_epoch(self, epoch_num):
        ...
        if True or self.vec_env.env.task.smpl_humanoid:
            humanoid_env = self.vec_env.env.task
            if (
                epoch_num > 1
            ) and epoch_num % humanoid_env.shape_resampling_interval == 1:  # + 1 to evade the evaluations.
    ...
    def calc_gradients(self, input_dict):
        ...
        if self.truncate_grads:
            if self.multi_gpu:
                ...
                self.scaler.step(self.optimizer)
                self.scaler.update()
                # print(self.model.a2c_network.pnn.actors[0][0].weight)
        else:
            self.scaler.step(self.optimizer)
            self.scaler.update()
    ...
    def _assemble_train_info(self, train_info, frame):
            ...
            train_info_dict.update(
                {
                    ...
                    "disc_agent_acc": torch_ext.mean_list(
                        train_info["disc_agent_acc"]
                    ).item(),
                    "disc_demo_acc": torch_ext.mean_list(
                        train_info["disc_demo_acc"]
                    ).item(),
                    "disc_agent_logit": torch_ext.mean_list(
                        train_info["disc_agent_logit"]
                    ).item(),
                    "disc_demo_logit": torch_ext.mean_list(
                        train_info["disc_demo_logit"]
                    ).item(),
                    "disc_grad_penalty": torch_ext.mean_list(
                        train_info["disc_grad_penalty"]
                    ).item(),
                    "disc_logit_loss": torch_ext.mean_list(
        ...
        if "sym_loss" in train_info:
            train_info_dict["sym_loss"] = torch_ext.mean_list(
                train_info["sym_loss"]
            ).item()
