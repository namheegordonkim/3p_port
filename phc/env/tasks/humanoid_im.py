...
import torch
import numpy as np
from phc.utils.torch_utils import quat_to_tan_norm, quat_to_exp_map, exp_map_to_quat
...
class HumanoidIm(humanoid_amp_task.HumanoidAMPTask):
    ...
    def __init__(
        ...
        self.custom_3p_ref_pos = None
        self.custom_3p_ref_rot = None
    ...
    def create_o3d_viewer(self):
        ...
        if self.viewer_o3d:
            ...
            if self.humanoid_type in ["smpl", "smplh", "smplx"]:
                from uhc.smpllib.smpl_joint_names import (
        ...
        if self.viewer_o3d and self.control_i == 0:
            ...
            if self.recording_state_change_o3d:
                if not self.recording:
                    ...
                    height, width = height if height % 2 == 0 else height - 1, (
            ...
            motion_lib_cfg = EasyDict(
                {
                    ...
                    "device": self.device,
                    # "curr_motion_path": self.cfg["curr_motion_path"],
                }
            )
    ...
    def _compute_observations(self, env_ids=None):
        ...
        #     # MYCODE: noise added only to tracking targets;
        #     # add angular noise in exponential map space
        #     obs = obs + torch.randn_like(obs) * 0.1
        #     obs[:, self._track_bodies_id]
    ...
    def _compute_task_obs(self, env_ids=None, save_buffer=True):
        ...
        # Overwrite only for inference when we have custom stuff provided
        if self.custom_3p_ref_pos is not None:
            ref_rb_pos = self.custom_3p_ref_pos[self.progress_buf, :]
            ref_rb_rot = self.custom_3p_ref_rot[self.progress_buf, :]
        # ref_rb_vel = self.custom_3p_ref_vel[self.progress_buf, :]
        # ref_rb_ang = self.custom_3p_ref_ang[self.progress_buf, :]
        ...
        # ref_rb_pos_subset = self.custom_3p_ref[self.progress_buf, :, :3]
        # ref_rb_rot_subset = self.custom_3p_ref[self.progress_buf, 1]

        # Noise augmentations
        if self.add_obs_noise and not flags.test:
            # MYCODE: noise added only to tracking targets;
            # add angular noise in exponential map space
            ref_rb_rot_subset_expm = quat_to_exp_map(ref_rb_rot_subset)
            ref_rb_pos_subset += torch.randn_like(ref_rb_pos_subset) * 0.1
            ref_rb_rot_subset_expm += torch.randn_like(ref_rb_rot_subset_expm) * 0.1
            ref_rb_rot_subset = exp_map_to_quat(ref_rb_rot_subset_expm)
